...
const ROCKET_RATE = 6, CHICKEN_RATE = 8;
const MAX_CHICKEN_DENSITY = 60; // limite max poulets à l’écran
let state = 'start';
let canvas;
let introBackgroundIdx = 0;
let mainMusic; // musique de jeu
...
function draw() {
  ...
  // liseret rouge autour du canvas
  noFill();
  stroke(233, 46, 46);
  strokeWeight(4);
...
function drawPlay() {
  ...
  // densité limitée des poulets
  if (enemies.length < MAX_CHICKEN_DENSITY) {
    let enemyFrequency = max(20, 120 - score * 2);
  ...
  // pics spawn sans limite densité mais vitesse différente
  let picFrequency = max(30, 180 - score * 1.5);
  if (frameCount % picFrequency === 0) {
    obstacles.push(makePic());
  }

  // vitesse progressive
  let chickenSpeed = 2 + score * 0.05;
  let picSpeed = 1.5 + score * 0.03;
