...
function setup() {
  ...
  // Pointer hand Spotify
  canvas.style('cursor','default');
  canvas.mouseMoved(()=>{
    canvas.style('cursor',
      mouseX >= SPOTIFY_BTN.x && mouseX <= SPOTIFY_BTN.x + SPOTIFY_BTN.w &&
...
function centerCanvas(){
  ...
  canvas.position((windowWidth-cw)/2,(windowHeight-ch)/2);
}
...
function drawStart(){
  ...
  let logoW=W*0.8, logoH=logo.height*(logoW/logo.width);
  image(logo,W/2-logoW/2,100,logoW,logoH);
  fill(233,46,46);
  textSize(36); text('FLAPPY FLYMAN',W/2,100+logoH+50);
...
function drawPlay(){
  ...
  let picSpeed=min(SPEED+score*0.05, SPEED+40*0.05);
  let chickenSpeed=min(picSpeed*1.3, (SPEED+40*0.05)*1.3);
  let enemyFreq=max(60,120-score*1.5);

  if(frameCount%enemyFreq===0){
...
function drawCenteredWrappedText(txt, centerX, centerY, maxWidth, lineHeight=20){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    text(line, centerX-lw/2, startY+i*lineHeight);
  }
  pop();
}
