...
function drawPlay(){
  ...
  for(let i=enemies.length-1;i>=0;i--){
    ...
    if(score>=20){
      if(c.vx===undefined){
        if(random()<0.5){ c.vx=-chickenSpeed/Math.sqrt(2); c.vy=chickenSpeed/Math.sqrt(2); }
        else{ c.vx=-chickenSpeed/Math.sqrt(2); c.vy=-chickenSpeed/Math.sqrt(2); }
        // 50% chance de voler droit
        if(random()<0.5){ c.vx=-chickenSpeed; c.vy=0; }
      }
      c.x += c.vx; c.y += c.vy;
...
function makePic(){
  ...
  const img = picImgs[idx]; let y,w,h,hitboxW;
  switch(picNames[idx]){
...
function mousePressed(){
  if(state==='over'){
    if(mouseX>=SPOTIFY_BTN.x && mouseX<=SPOTIFY_BTN.x+SPOTIFY_BTN.w &&
       mouseY>=SPOTIFY_BTN.y && mouseY<=SPOTIFY_BTN.y+SPOTIFY_BTN.h){ window.open(SPOTIFY_URL,'_blank'); return; }
  }
  action();
}
...
function drawCenteredWrappedText(txt,centerX,centerY,maxWidth,lineHeight=20){
  ...
  for(let i=0;i<lines.length;i++){
    const lw = textWidth(lines[i]);
    text(lines[i], centerX-lw/2, startY+i*lineHeight);
  }
  pop();
}
