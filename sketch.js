// Flappy Flyman – version optimisée + diagonales poulets aléatoires + son ok
...
function centerCanvas() {
  ...
  canvas.position((windowWidth - W*scaleFactor)/2, (windowHeight - H*scaleFactor)/2);
}
...
function drawStart() {
  ...
  let logoWidth = W*0.8, logoHeight = logo.height*(logoWidth/logo.width);
...
function drawOver() {
  ...
  let bg = backgroundGameFrames[backgroundGameIdx], bgW = bg.width*0.25, bgH = bg.height*0.25;
  image(bg, (W-bgW)/2, H-bgH, bgW, bgH);
  ...
  push();
  noStroke();
  if(spotifyLogoImg) imageMode(CENTER), image(spotifyLogoImg, SPOTIFY_BTN.x + SPOTIFY_BTN.w/2, SPOTIFY_BTN.y + SPOTIFY_BTN.h/2);
  if(mouseX>=SPOTIFY_BTN.x && mouseX<=SPOTIFY_BTN.x+SPOTIFY_BTN.w && mouseY>=SPOTIFY_BTN.y && mouseY<=SPOTIFY_BTN.y+SPOTIFY_BTN.h) cursor('pointer');
  else cursor('default');
  pop();
}
...
function drawPlay() {
  ...
  let picSpeed=SPEED+score*0.05, chickenSpeed=picSpeed*1.3;
  let enemyFreq = max(60,120-score*1.5);
  if(frameCount%enemyFreq===0) random()>0.5 ? enemies.push(makeChicken()) : obstacles.push(makePic());

  for(let i=enemies.length-1;i>=0;i--){
    let c=enemies[i];
    if(score>=20 && c.vx===undefined){
      const v=chickenSpeed/Math.sqrt(2);
      if(random()<0.5){ c.vx=-v; c.vy=v; } else { c.vx=-v; c.vy=-v; }
    }
    if(c.vx!==undefined){ c.x+=c.vx; c.y+=c.vy; if(c.y<0){c.y=0;c.vy=-c.vy} else if(c.y+c.h>H){c.y=H-c.h;c.vy=-c.vy} }
...
function drawChicken(c){ push(); imageMode(CENTER); if(frameCount%(60/CHICKEN_RATE)===0) chickenIdx=(chickenIdx+1)%chickenFrames.length; image(chickenFrames[chickenIdx],c.x+25,c.y+25,50,50); pop(); }
function drawPic(p){ image(p.img,p.x,p.y,p.w,p.h); }
function makeChicken(){ return {x:W,y:random(25,H-25),w:50,h:50,passed:false}; }
function makePic(){ const idx=floor(random(4)),img=picImgs[idx]; let y,w,h,hitboxW; switch(picNames[idx]){ case 'pic_petit_haut.png': w=h=80*1.05; hitboxW=20; y=0; break; case 'pic_petit_bas.png': w=h=80*1.05; hitboxW=20; y=H-h; break; case 'pic_gros_haut.png': w=h=120*1.05; hitboxW=40; y=0; break; case 'pic_gros_bas.png': w=h=120*1.05; hitboxW=40; y=H-h; break; } return {x:W,y,w,h,hitboxW,img,passed:false}; }
...
function action(){ if(state==='start'){resetGame();state='play';startMusic();} else if(state==='play'){rocket.vel=FLAP;} else if(state==='over'){resetGame();state='play';startMusic();} }
function keyPressed(){ if(key===' ') action(); }
function mousePressed(){ if(state==='over' && mouseX>=SPOTIFY_BTN.x && mouseX<=SPOTIFY_BTN.x+SPOTIFY_BTN.w && mouseY>=SPOTIFY_BTN.y && mouseY<=SPOTIFY_BTN.y+SPOTIFY_BTN.h){ window.open(SPOTIFY_URL,'_blank'); return;} action(); }
...
function drawCenteredWrappedText(txt, centerX, centerY, maxWidth, lineHeight=20){
  ...
  for(let i=0;i<words.length;i++){
    let testLine=currentLine?currentLine+' '+words[i]:words[i]; let tw=textWidth(testLine);
    if(tw>maxWidth){ if(currentLine) lines.push(currentLine); currentLine=words[i]; } else { currentLine=testLine; }
  } if(currentLine) lines.push(currentLine);
  const totalH=lines.length*lineHeight; let startY=centerY-totalH/2+lineHeight/2;
  textAlign(LEFT,CENTER); for(let i=0;i<lines.length;i++){ const line=lines[i], lw=textWidth(line), x=centerX-lw/2, y=startY+i*lineHeight; text(line,x,y); }
  pop();
}
